...
struct StaveNoteTests {
    ...
    @Test func staveNoteRest() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .b, octave: 4)], duration: .quarter, type: .rest))
        #expect(note.isRest())
    ...
    @Test func staveNoteChord() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4), StaffKeySpec(letter: .e, octave: 4), StaffKeySpec(letter: .g, octave: 4)], duration: .quarter))
        #expect(note.isChord())
    ...
    @Test func staveNoteWhole() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .whole))
        #expect(!note.hasStem())
        #expect(!note.hasFlag())
    }
    ...
    @Test func staveNoteSixteenth() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .sixteenth))
        #expect(note.hasStem())
        #expect(note.hasFlag())
    ...
    @Test func autoStemUp() {
        ...
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter, autoStem: true))
        #expect(note.getStemDirection() == Stem.UP)
    }

    @Test func autoStemDown() {
        // Notes above middle line -> stem down
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .a, octave: 5)], duration: .quarter, autoStem: true))
        #expect(note.getStemDirection() == Stem.DOWN)
    }

    @Test func autoStemChord() {
        // Chord spanning middle: average determines direction
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4), StaffKeySpec(letter: .a, octave: 5)], duration: .quarter, autoStem: true))
    ...
    @Test func stemDirectionExplicit() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter, stemDirection: Stem.DOWN))
        #expect(note.getStemDirection() == Stem.DOWN)
    }
    ...
    @Test func keyPropsCalculation() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4), StaffKeySpec(letter: .e, octave: 4), StaffKeySpec(letter: .g, octave: 4)], duration: .quarter, clef: .treble))
        #expect(note.keyProps.count == 3)
    ...
    @Test func displacementDetection() {
        ...
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .e, octave: 4), StaffKeySpec(letter: .f, octave: 4)], duration: .quarter))
        #expect(note.displaced)
    }

    @Test func noDisplacement() {
        // C/4 and E/4 are well separated -> no displacement
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4), StaffKeySpec(letter: .e, octave: 4)], duration: .quarter))
        #expect(!note.displaced)
    }
    ...
    @Test func lineNumber() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4), StaffKeySpec(letter: .g, octave: 4)], duration: .quarter, clef: .treble))
    ...
    @Test func isChordCheck() {
        let single = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter))
        let chord = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4), StaffKeySpec(letter: .e, octave: 4)], duration: .quarter))
        let rest = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .b, octave: 4)], duration: .quarter, type: .rest))
    ...
    @Test func keyLine() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter, clef: .treble))
    ...
    @Test func formatterSimpleFormat() {
        let notes: [Tickable] = [
            ...
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .f, octave: 4)], duration: .quarter)),
        ]
    ...
    @Test func formatterResolutionMultiplier() {
        ...
        let notes2: [Tickable] = [
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .g, octave: 4)], duration: .half)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .a, octave: 4)], duration: .half)),
        ]
    ...
    @Test func mixedDurations() {
        ...
        let notes: [Tickable] = [
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .half)),
            ...
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .g, octave: 4)], duration: .quarter)),
        ]
        _ = voice.addTickables(notes)
    ...
    @Test func dottedNotesFormat() {
        ...
        let note1 = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter))
        Dot.buildAndAttach([note1])
        let note2 = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .e, octave: 4)], duration: .eighth))
        let note3 = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .g, octave: 4)], duration: .half))
    ...
    @Test func twoVoicesFormat() {
        ...
        let notes1: [Tickable] = [
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 5)], duration: .quarter, stemDirection: Stem.UP)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .d, octave: 5)], duration: .quarter, stemDirection: Stem.UP)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .e, octave: 5)], duration: .quarter, stemDirection: Stem.UP)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .f, octave: 5)], duration: .quarter, stemDirection: Stem.UP)),
        ]
        let notes2: [Tickable] = [
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter, stemDirection: Stem.DOWN)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .d, octave: 4)], duration: .quarter, stemDirection: Stem.DOWN)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .e, octave: 4)], duration: .quarter, stemDirection: Stem.DOWN)),
            StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .f, octave: 4)], duration: .quarter, stemDirection: Stem.DOWN)),
        ]
        _ = voice1.addTickables(notes1)
        _ = voice2.addTickables(notes2)
    ...
    @Test func bassClefNote() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 3)], duration: .quarter, clef: .bass))
        #expect(note.clef == .bass)
        #expect(note.keyProps.count == 1)
    }

    @Test func altoClefNote() {
        let note = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter, clef: .alto))
    ...
    @Test func staveNoteStaticFormat() {
        ...
        let n1 = StaveNote(StaveNoteStruct(keys: [StaffKeySpec(letter: .c, octave: 4)], duration: .quarter))
