...
struct Phase8Tests {
    ...
    private func makeNote(keys: [String], duration: NoteValue) -> StaveNote {
    ...
    @Test func staveTieCreation() {
        let note1 = makeNote(keys: ["c/4"], duration: .quarter)
        let note2 = makeNote(keys: ["c/4"], duration: .quarter)
    ...
    @Test func staveTiePartialFirst() {
        let note = makeNote(keys: ["c/4"], duration: .quarter)
    ...
    @Test func staveTieIndices() {
        let note1 = makeNote(keys: ["c/4", "e/4", "g/4"], duration: .quarter)
        let note2 = makeNote(keys: ["c/4", "e/4", "g/4"], duration: .quarter)
    ...
    @Test func curveCreation() {
        let note1 = makeNote(keys: ["c/4"], duration: .quarter)
        let note2 = makeNote(keys: ["e/4"], duration: .quarter)
    ...
    @Test func curveSetNotes() {
        let note1 = makeNote(keys: ["c/4"], duration: .quarter)
        let note2 = makeNote(keys: ["e/4"], duration: .quarter)
        let note3 = makeNote(keys: ["g/4"], duration: .quarter)
        let curve = Curve(from: note1, to: note2)
    ...
    @Test func tupletRatioed() {
        ...
        let notes5: [Note] = [
            ...
            StaveNote(StaveNoteStruct(keys: ["g/4"], duration: .eighth)),
        ]
    ...
    @Test func tupletYPositionBottom() {
        let stave = Stave(x: 10, y: 40, width: 300)
        let notes: [Note] = [
            StaveNote(StaveNoteStruct(keys: ["c/5"], duration: .eighth)),
            StaveNote(StaveNoteStruct(keys: ["d/5"], duration: .eighth)),
            StaveNote(StaveNoteStruct(keys: ["e/5"], duration: .eighth)),
        ]
        for note in notes {
            _ = note.setStave(stave)
            if let sn = note as? StaveNote {
